...
def hot_picks(
    ...
    mood = mood.strip()
    try:
        tracks = _query_catalog(db, search, genre, mood, None, None)
        tracks = [track for track in tracks if _track_is_public(track)][:12]
    except Exception:
    ...
    for track in tracks:
        catalog.append({
            "track": track,
            "title": str(_model_value(track, "title", "name", default="Untitled Beat")),
            "producer": _producer_name(track),
            "price": _track_price(track),
            "artwork_url": _track_artwork(track),
            "audio_url": _track_audio(track),
            "url": _track_url(track),
            "genre": str(_model_value(track, "genre", "category", default="") or ""),
            "mood": str(_model_value(track, "mood", default="") or ""),
            "bpm": str(_model_value(track, "bpm", "tempo", default="") or ""),
            "key": str(_model_value(track, "key", "musical_key", default="") or ""),
            "description": str(_model_value(track, "description", "short_description", default="") or ""),
        })
    return templates.TemplateResponse(request, "beats.html", {
        "request": request, "current_user": current_user, "user": current_user,
        "current_year": 2026, "tracks": tracks, "beats": tracks, "catalog": catalog,
        "total": len(tracks), "total_results": len(tracks), "page": 1, "track_page": 1,
        "per_page": 12, "track_per_page": 12, "total_pages": 1, "track_total_pages": 1,
        "genres": [], "moods": [], "query": search, "q": search, "genre": genre, "mood": mood,
        "min_price": None, "max_price": None, "has_previous": False, "has_next": False,
        "previous_page": 1, "next_page": 1, "catalog_start": 1 if tracks else 0,
        "catalog_end": len(tracks), "title": "Hot Picks",
    })
