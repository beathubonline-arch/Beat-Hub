...
Merchandise uses the same Paystack payment gateway as beat purchases.
Daraja/M-Pesa SDK calls are intentionally absent here. Paystack Checkout
handles the customer's available Kenya payment methods, including cards and
M-PESA, while the server verifies the final transaction before marking an
order paid.
"""
...
def ensure_merch_tables(db: Session) -> None:
    """Create/upgrade the small merchandise tables without a destructive migration."""
    db.execute(
        text(
            f"""
    ...
    # Additive compatibility columns. Existing test/live merchandise orders
    # remain readable while new orders carry an authoritative 10/90 split.
    for name, definition in (
        ("commission_amount", "NUMERIC(12,2)"),
        ("net_amount", "NUMERIC(12,2)"),
        ("commission_percent_at_purchase", "NUMERIC(5,2)"),
    ...
    db.execute(
        text(
            f"CREATE INDEX IF NOT EXISTS idx_{MERCH_TABLE}_creator ON {MERCH_TABLE}(creator_profile_id)"
        )
    )
    db.execute(
        text(
            f"CREATE INDEX IF NOT EXISTS idx_{MERCH_ORDER_TABLE}_buyer ON {MERCH_ORDER_TABLE}(buyer_id)"
        )
    )
    db.execute(
        text(
            f"CREATE INDEX IF NOT EXISTS idx_{MERCH_ORDER_TABLE}_status ON {MERCH_ORDER_TABLE}(status)"
        )
    )
    db.commit()
...
def _image_url(request: Request, path: str | None) -> str | None:
    if not path:
        return None
    try:
...
def _delete_storage(path: str | None) -> None:
    ...
    try:
        if value.startswith(("r2://", "s3://")):
            normalized = value.replace("s3://", "r2://", 1)
...
def _products(db: Session, creator_profile_id: str | None = None):
    if creator_profile_id:
        rows = db.execute(
            ...
            {"profile_id": str(creator_profile_id)},
        ).mappings().all()
    else:
        rows = db.execute(
            text(
...
def merch_dashboard(request: Request, db: Session = Depends(get_db), user: User = Depends(require_creator)):
    ...
    return templates.TemplateResponse(
        request,
        "merchandise.html",
        _ctx(request, user, profile=profile, products=products, product_count=len(products),
...
async def merch_create(
    ...
    def error(message: str):
        return templates.TemplateResponse(
            ...
            _ctx(request, user, error=message, name=name, description=description, price=price_raw),
            status_code=400,
        )
    ...
    if len(description) > 4000:
        return error("Product description is too long.")
    ...
    if extension not in ALLOWED_IMAGE_EXT:
        return error("Unsupported image type. Use JPG, JPEG, PNG or WEBP.")

    try:
        if _r2_is_configured():
            image_path = await save_upload_to_r2(image, "merch", ALLOWED_IMAGE_EXT)
        else:
            image_path = await save_upload(image, "merch", ALLOWED_IMAGE_EXT)
    except UploadValidationError as exc:
        return error(str(exc))
    except Exception:
        return error("Product image upload failed. Please try again.")
        ...
        db.execute(
            text(
                ...
                INSERT INTO {MERCH_TABLE}
                    (id, creator_profile_id, name, slug, description, price, image_path)
                VALUES
                    (:id, :profile_id, :name, :slug, :description, :price, :image_path)
                """
            ),
            {
                "id": product_id,
...
def merch_delete(product_id: str, db: Session = Depends(get_db), user: User = Depends(require_creator)):
    ...
    row = db.execute(
        text(f"SELECT id, image_path FROM {MERCH_TABLE} WHERE id=:id AND creator_profile_id=:profile_id LIMIT 1"),
        {"id": str(product_id), "profile_id": str(profile.id)},
    ).mappings().first()
    if not row:
    ...
    db.execute(text(f"DELETE FROM {MERCH_TABLE} WHERE id=:id AND creator_profile_id=:profile_id"),
               {"id": str(product_id), "profile_id": str(profile.id)})
    db.commit()
    _delete_storage(row["image_path"])
...
def legacy_merch_media(filename: str):
    ...
    root = Path(getattr(settings, "MEDIA_ROOT", "media") or "media").resolve()
    path = (root / "merch" / clean).resolve()
    try:
...
def merch_marketplace(request: Request, db: Session = Depends(get_db), user: Optional[User] = Depends(get_optional_user)):
    ...
    return templates.TemplateResponse(
        ...
        _ctx(request, user, products=products, title="BeatHub Merch", creator=None, store_url=None,
             product_detail=False, query_error=request.query_params.get("error")),
    )
...
def creator_merch_store(request: Request, slug: str, db: Session = Depends(get_db), user: Optional[User] = Depends(get_optional_user)):
    ...
    return templates.TemplateResponse(
        request,
        "merchandise_public.html",
        _ctx(request, user, products=products, title=f"{profile.stage_name} — Merch", creator=profile,
             store_url=f"/store/{profile.slug}", product_detail=False, query_error=request.query_params.get("error")),
    )
...
def merch_detail(request: Request, slug: str, db: Session = Depends(get_db), user: Optional[User] = Depends(get_optional_user)):
    ...
    if row is None:
        raise HTTPException(status_code=404, detail="Merchandise item not found.")
    product = dict(row)
    owner = db.query(Profile).filter(Profile.id == str(product["creator_profile_id"])).first()
    product["image_url"] = _image_url(request, product.get("image_path"))
    product["creator"] = owner
    return templates.TemplateResponse(
        request,
        "merchandise_public.html",
        _ctx(request, user, products=[product], title=product["name"], creator=owner,
             store_url=f"/store/{owner.slug}" if owner else None, product_detail=True,
...
async def buy_merchandise(
    ...
    """Create a Paystack checkout for merchandise.

    The hosted checkout exposes the payment channels enabled for the Kenya
    Paystack account. We explicitly allow card + mobile_money so M-PESA and
    cards remain available through the same payment flow.
    """
    ensure_merch_tables(db)
    row = _product(db, slug)
    if row is None:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Merchandise pricing could not be calculated: {exc}")
    ...
    db.execute(
        text(
            ...
            (id, product_id, buyer_id, quantity, unit_price, total_amount, phone_number,
             order_note, status, commission_amount, net_amount, commission_percent_at_purchase,
             payment_provider)
            VALUES
            (:id, :product_id, :buyer_id, :quantity, :unit_price, :total_amount, :phone,
             :note, 'pending_payment', :commission, :net, :commission_percent, 'paystack')
            """
        ),
        {
            "id": order_id,
    ...
    payload = {
        ...
        "amount": str(_paystack_amount(total)),
        "currency": "KES",
        "reference": order_number,
        "callback_url": callback_url,
        "channels": ["card", "mobile_money"],
        "metadata": {
            "beathub_merchandise_order_id": order_id,
            "beathub_merchandise_product_id": str(row["id"]),
            "beathub_merchandise_order_number": order_number,
            ...
            "buyer_id": str(user.id),
            "customer_phone": (phone or "").strip()[:32],
        },
    }
    ...
    try:
        ...
        db.execute(text(f"UPDATE {MERCH_ORDER_TABLE} SET status='failed', failure_reason=:reason WHERE id=:id"),
                   {"reason": "Paystack could not be reached.", "id": order_id})
        db.commit()
    ...
    if not authorization_url:
        db.execute(text(f"UPDATE {MERCH_ORDER_TABLE} SET status='failed', failure_reason=:reason WHERE id=:id"),
                   {"reason": "Paystack did not return a checkout URL.", "id": order_id})
        db.commit()
    ...
    db.execute(
        text(
            ...
            SET checkout_request_id=:reference
            WHERE id=:id AND status='pending_payment'
            """
        ),
        {"reference": str(reference), "id": order_id},
    )
    db.commit()
...
def merchandise_order_status(
    ...
    row = db.execute(
        text(
            f"""
            SELECT o.*, m.name AS product_name, m.slug AS product_slug, m.image_path,
...
async def legacy_paystack_merchandise_callback(request: Request, db: Session = Depends(get_db)):
    """Compatibility callback for old Paystack merchandise checkouts.

    New checkouts use the shared /paystack/callback and /paystack/webhook
    handlers. This route remains only so an older in-flight checkout cannot
    strand an order during deployment.
    """
