...
from app.services.storage import ALLOWED_AUDIO_EXT, ALLOWED_IMAGE_EXT, UploadValidationError, _r2_is_configured, save_upload, save_upload_to_r2
...
async def publish_tracks(
    ...
    titles: List[str] = Form(...), descriptions: List[str] = Form(...), genres: List[str] = Form(...),
    bpms: Optional[List[str]] = Form(None), tags_list: List[str] = Form(...), prices: List[str] = Form(...),
    currencies: List[str] = Form(...), sales_models: List[str] = Form(...), content_types: List[str] = Form(...),
    audio_files: List[UploadFile] = File(...), cover_files: List[Optional[UploadFile]] = File(None),
):
    ...
    lengths = {"titles": len(titles), "descriptions": len(descriptions), "genres": len(genres), "tags": len(tags_list), "prices": len(prices), "currencies": len(currencies), "sales_models": len(sales_models), "content_types": len(content_types)}
    if any(v != expected for v in lengths.values()):
        return _error(request, user, "Each uploaded item must include its currency, content type and required details.")
    bpms = bpms or []
    ...
    try:
        for i, audio_file in enumerate(audio_files):
            ...
            audio_path = await (save_upload_to_r2(audio_file, "audio", ALLOWED_AUDIO_EXT) if _r2_is_configured() else save_upload(audio_file, "audio", ALLOWED_AUDIO_EXT))
            cover_path = None
            if cover_files and i < len(cover_files) and cover_files[i] is not None and cover_files[i].filename:
                cover_path = await (save_upload_to_r2(cover_files[i], "covers", ALLOWED_IMAGE_EXT) if _r2_is_configured() else save_upload(cover_files[i], "covers", ALLOWED_IMAGE_EXT))

            track = Track(creator_profile_id=profile.id, title=title, slug=unique_slug(db, Track, title, "track"), description=(descriptions[i] or "").strip() or None, genre=(genres[i] or "").strip() or None, bpm=bpm_value, tags=(tags_list[i] or "").strip() or None, audio_file_path=audio_path, cover_art_path=cover_path, price=price_value, currency=currency, sales_model=sales_model, content_type=content_raw, is_published=True)
            db.add(track)
    ...
    return RedirectResponse(url="/dashboard?success=" + f"{len(created)} {item_word} published successfully.".replace(" ", "%20"), status_code=303)
