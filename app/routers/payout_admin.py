...
from app.models.ledger import CreatorLedgerEntry, WithdrawalRequest
...
def _redirect(message: str, error: bool = False):
    ...
    return RedirectResponse(url=f"/admin/withdrawals?{key}={quote(message)}", status_code=303)
...
def _record_creator_withdrawal_debit(db: Session, withdrawal: WithdrawalRequest) -> bool:
    """Create exactly one negative creator-ledger entry for a confirmed payout."""
    existing = (
        db.query(CreatorLedgerEntry)
        .filter(CreatorLedgerEntry.withdrawal_request_id == withdrawal.id)
        .first()
    )
    if existing:
        return False
    ...
    db.add(
        CreatorLedgerEntry(
            creator_profile_id=withdrawal.creator_profile_id,
            withdrawal_request_id=withdrawal.id,
            amount=-Decimal(str(withdrawal.amount or 0)),
            description=f"Creator withdrawal {withdrawal.id} paid to M-Pesa",
        )
    )
    return True
...
def approve_creator_withdrawal_confirmed(
    ...
    if approved_amount != requested_amount:
        return _redirect(f"Approval amount must match the producer request of KSh {requested_amount:.2f}.", error=True)
...
def mark_creator_withdrawal_processing(withdrawal_id: str, db: Session = Depends(get_db), user=Depends(require_admin)):
    if not is_payout_window():
        return _redirect("Creator payouts can only be moved to processing on Tuesday or Thursday before 6:00 PM EAT.", error=True)
    withdrawal = _withdrawal_or_404(withdrawal_id, db)
...
def mark_creator_withdrawal_paid(
    ...
    if not is_payout_window():
        return _redirect("Creator payouts can only be completed on Tuesday or Thursday before 6:00 PM EAT.", error=True)
    ...
    withdrawal = _withdrawal_or_404(withdrawal_id, db)
    current = str(withdrawal.status).lower()
    if current == "paid":
        return _redirect("This creator withdrawal is already paid; no duplicate ledger debit was created.", error=True)
    if current not in ("approved", "processing"):
    ...
    _record_creator_withdrawal_debit(db, withdrawal)
    db.commit()
    return _redirect("Creator withdrawal marked as paid and debited from the creator ledger.")
