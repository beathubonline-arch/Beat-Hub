...
def dashboard_home(
    ...
    # Search works across title, genre and tags.
    # Pagination keeps the dashboard fast even with thousands of tracks.
    # --------------------------------------------------------------
    page = max(page, 1)
    ...
    total_pages = max(1, ceil(total_tracks / TRACKS_PER_PAGE))
    ...
    # Public storefront/profile URL.
    # Buyers can open this page, browse the creator's music and purchase tracks.
    store_url = str(request.base_url).rstrip("/") + f"/profile/{profile.slug}"
...
def new_album_page(
    ...
    return templates.TemplateResponse(
        request,
        "upload_album.html",
        ctx(request, user, tracks=tracks),
    )
