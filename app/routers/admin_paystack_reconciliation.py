...
from app.services.paystack_settlement import (
    PaystackSettlementError,
...
async def sync_reconciliation(
    from_date: str | None = Query(default=None, description="Paystack settlement start date/time"),
    to_date: str | None = Query(default=None, description="Paystack settlement end date/time"),
    status: str | None = Query(default=None, description="Paystack settlement status filter"),
    db: Session = Depends(get_db),
    _admin=Depends(require_admin),
):
    ...
    try:
        result = await reconcile_settlements(
            db,
            from_date=from_date,
            to_date=to_date,
            status=status,
