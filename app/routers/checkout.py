...
def track_is_available(track: Track) -> bool:
    if not track or not bool(getattr(track, "is_published", False)): return False
    sales_model = track_sales_model_value(track)
    if sales_model == SalesModel.NON_EXCLUSIVE.value: return True
    if sales_model == SalesModel.EXCLUSIVE.value: return not bool(getattr(track, "is_sold", False))
    return False
...
def checkout_page(slug: str, request: Request, db: Session = Depends(get_db), user: User | None = Depends(get_optional_user)):
    ...
    if user is None:
        next_url=f"/checkout/track/{quote(slug,safe='')}"; message=quote("Please log in to purchase this beat. After you sign in, your checkout will continue automatically.")
        return RedirectResponse(url=f"/login?next={quote(next_url,safe='')}&error={message}",status_code=303)
    profile=getattr(track,"creator_profile",None); creator_user_id=getattr(profile,"user_id",None)
...
async def order_status_api(order_id: str, db: Session = Depends(get_db), user: User = Depends(require_user)):
        ...
        if reference and settings.PAYSTACK_SECRET_KEY:
            try:
                async with httpx.AsyncClient(timeout=httpx.Timeout(6.0,connect=3.0)) as client:
                    response=await client.get(f"{settings.PAYSTACK_BASE_URL.rstrip('/')}/transaction/verify/{reference}",headers={"Authorization":f"Bearer {settings.PAYSTACK_SECRET_KEY}","Content-Type":"application/json"})
                if response.status_code < 400:
                    data=response.json().get("data") or {}
                    if str(data.get("status") or "").lower()=="success":
                        expected=int((order.gross_amount*100).quantize(1)); actual=int(data.get("amount") or 0); currency=str(data.get("currency") or "").upper()
                        if actual==expected and currency=="KES":
                            payment.status=PaymentStatus.COMPLETED; payment.result_code=0; payment.result_description="Paystack payment verified successfully."; payment.callback_processed=True; payment.completed_at=datetime.utcnow()
    ...
    return {"status":order.status.value,"order_number":order.order_number,"track_slug":track_slug,"completed":order.status==OrderStatus.COMPLETED,"failed":order.status==OrderStatus.FAILED,"rejected":order.status==OrderStatus.REJECTED}
