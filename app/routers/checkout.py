...
from app.routers.paystack_checkout import _complete_verified_payment
...
def checkout_page(
    ...
    if user is None:
        ...
        message = quote(
            "Please log in to purchase this beat. After you sign in, your checkout will continue automatically."
        )
        return RedirectResponse(
            url=f"/login?next={quote(next_url, safe='')}&error={message}",
            status_code=303,
        )
...
async def order_status_api(
    ...
    if order.status == OrderStatus.PENDING:
        ...
        if reference and settings.PAYSTACK_SECRET_KEY and payment:
            try:
    ...
    return {
        ...
        "currency": str(getattr(order, "currency", "KES") or "KES").upper(),
