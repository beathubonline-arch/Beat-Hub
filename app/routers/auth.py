...
def _cookie_secure() -> bool: return bool(settings.is_production)
def _set_auth_cookie(response: RedirectResponse, token: str) -> None: response.set_cookie(key=SESSION_COOKIE_NAME, value=token, httponly=True, max_age=COOKIE_MAX_AGE, samesite="lax", secure=_cookie_secure(), path="/")
def slugify(value: str) -> str:
    value = re.sub(r"[^a-zA-Z0-9]+", "-", (value or "").strip()).strip("-").lower(); return value or f"producer-{secrets.token_hex(4)}"
...
def _signup_context(request: Request, **extra):
    context = {"request": request, "stage_name": "", "email": "", "role": "buyer"}; context.update(extra); return context
def _reset_token_digest(token: str) -> str: return hashlib.sha256(token.encode("utf-8")).hexdigest()
...
def _new_verification_code() -> str: return f"{secrets.randbelow(1_000_000):06d}"
def _set_verification_code(user: User) -> str:
    code = _new_verification_code(); user.verification_code_hash = _verification_code_digest(code); user.verification_code_expires = datetime.utcnow() + VERIFICATION_CODE_TTL; user.verification_attempts = 0; return code
def _send_email_resend(to_email: str, subject: str, body: str, sender: str | None = None, reply_to: str | None = None) -> bool:
    api_key = str(getattr(settings, "RESEND_API_KEY", "") or "").strip(); sender = str(sender or getattr(settings, "RESEND_FROM", "") or getattr(settings, "EMAIL_FROM", "") or "").strip()
    if not api_key or not sender: logger.error("Resend email delivery is misconfigured: RESEND_API_KEY or sender is missing."); return False
    ...
    try:
        with httpx.Client(timeout=15.0) as client: response = client.post(RESEND_API_URL, json=payload, headers=headers)
        if 200 <= response.status_code < 300: logger.info("Email sent successfully through Resend from configured transactional sender."); return True
        error_name = ""; error_message = ""
        try:
            error_data = response.json()
            if isinstance(error_data, dict): error_name = str(error_data.get("name", ""))[:120]; error_message = str(error_data.get("message", ""))[:240]
...
def _send_email(to_email: str, subject: str, body: str, sender: str | None = None, reply_to: str | None = None) -> bool:
    if not bool(getattr(settings, "EMAIL_ENABLED", False)): logger.warning("Email delivery disabled by EMAIL_ENABLED."); return False
    ...
    logger.error("Unsupported email provider: %s", provider); return False
def _send_verification_email(email: str, code: str) -> bool:
    sender = str(getattr(settings, "EMAIL_VERIFICATION_FROM", "") or "BeatHub <no-reply@mybeathub.com>").strip(); return _send_email(email, "Verify your BeatHub email", "Welcome to BeatHub.\n\n" f"Your verification code is: {code}\n\nThis code expires in 10 minutes and can only be used once.\n\nIf you did not create this account, you can ignore this email.", sender=sender)
def _send_password_reset_email(email: str, reset_url: str) -> bool:
    sender = str(getattr(settings, "PASSWORD_RESET_FROM", "") or "BeatHub Password Reset <reset-password@mybeathub.com>").strip(); reply_to = str(getattr(settings, "SUPPORT_EMAIL", "") or "support@mybeathub.com").strip(); return _send_email(email, "BeatHub password reset", "We received a request to reset your BeatHub password.\n\n" f"Use this link within 1 hour:\n{reset_url}\n\nIf you did not request this, you can safely ignore this email.", sender=sender, reply_to=reply_to)
def _verification_delivery_error_message() -> str: return "We couldn't send your verification email right now. Please try again later."
def _prepare_verification_code() -> str: return _new_verification_code()
def _store_verification_code(user: User, code: str) -> None:
    user.verification_code_hash = _verification_code_digest(code); user.verification_code_expires = datetime.utcnow() + VERIFICATION_CODE_TTL; user.verification_attempts = 0

@router.get("/signup")
def signup_page(request: Request, role: str = "buyer"):
    role = role.strip().lower(); role = role if role in {"artist", "creator", "buyer"} else "buyer"; return templates.TemplateResponse(request, "signup.html", _signup_context(request, role=role))
...
def login_page(request: Request, next: str = "", error: str = "", success: str = ""):
    safe_next = _safe_next_url(next); return templates.TemplateResponse(request, "login.html", {"request": request, "next": safe_next, "next_url": safe_next, "error": error, "success": success})
@router.post("/login")
def login_submit(request: Request, identifier: str = Form(""), email: str = Form(""), password: str = Form(...), next: str = Form(""), db: Session = Depends(get_db)):
    requested_next = next or request.query_params.get("next", ""); safe_next = _safe_next_url(requested_next); login_identifier = (identifier or email).strip().lower(); user = db.query(User).filter(or_(func.lower(User.email) == login_identifier, func.lower(User.username) == login_identifier)).first()
...
def forgot_password_submit(request: Request, db: Session = Depends(get_db), email: str = Form(...)):
    ...
    if user:
        token = secrets.token_urlsafe(32); user.reset_token = _reset_token_digest(token); user.reset_token_expires = datetime.utcnow() + RESET_TOKEN_TTL; db.commit(); base_url = str(getattr(settings, "BASE_URL", "") or "").strip().rstrip("/") or str(request.base_url).rstrip("/"); reset_url = f"{base_url}/reset-password?token={quote(token, safe='')}"; delivered = _send_password_reset_email(email_norm, reset_url)
...
def reset_password_page(request: Request, token: str, db: Session = Depends(get_db)):
    token_digest = _reset_token_digest(token); user = db.query(User).filter(User.reset_token == token_digest).first(); valid = bool(user and user.reset_token_expires and user.reset_token_expires > datetime.utcnow()); return templates.TemplateResponse(request, "reset_password.html", {"request": request, "token": token, "valid": valid, "error": ""})
...
def reset_password_submit(request: Request, db: Session = Depends(get_db), token: str = Form(...), password: str = Form(...), confirm_password: str = Form(...)):
    ...
    user.hashed_password = hash_password(password); user.reset_token = None; user.reset_token_expires = None; db.commit(); return RedirectResponse(url="/login?success=Password%20updated.%20Please%20log%20in.", status_code=303)
