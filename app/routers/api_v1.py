...
def api_signup(payload:SignupIn,db:Session=Depends(get_db)):
    ...
    base=re.sub(r"[^a-zA-Z0-9]","",payload.stage_name.lower())[:90] or f"user{uuid.uuid4().hex[:8]}"; username,suffix=base,2
    ...
    code=_prepare_verification_code(); _store_verification_code(user,code); db.add(user); db.add(profile); db.commit()
...
def api_resend_verification(payload:ResendVerificationIn,db:Session=Depends(get_db)):
