...
class SignupIn(BaseModel): stage_name: str = Field(min_length=1,max_length=120); email: EmailStr; password: str = Field(min_length=8); role: str = Field(default="buyer",pattern="^(buyer|creator|artist)$")
class PaymentIn(BaseModel): slug: str = Field(min_length=1,max_length=255); email: EmailStr | None = None

def _role(user): return str(getattr(getattr(user,"role",None),"value",getattr(user,"role","buyer")))
...
def _track_payload(track):
    profile=getattr(track,"creator_profile",None); sales=getattr(getattr(track,"sales_model",None),"value",track.sales_model)
    return {"id":track.id,"title":track.title,"slug":track.slug,"description":track.description,"genre":track.genre,"bpm":track.bpm,"price":float(track.price),"currency":track.currency,"sales_model":str(sales),"is_sold":bool(track.is_sold),"artwork_url":f"/track/{track.slug}/artwork","preview_url":f"/track/{track.slug}/preview","track_url":f"/track/{track.slug}","producer":getattr(profile,"stage_name",None),"producer_slug":getattr(profile,"slug",None)}
...
def api_signup(payload:SignupIn,db:Session=Depends(get_db)):
    ...
    role_name="creator" if payload.role in {"creator","artist"} else "buyer"; base=re.sub(r"[^a-zA-Z0-9]","",payload.stage_name.lower())[:90] or f"user{uuid.uuid4().hex[:8]}"; username=base; suffix=2
    while db.query(User).filter(User.username.ilike(username)).first():username=f"{base}{suffix}";suffix+=1
    ...
    db.add(user);db.add(profile);db.commit();return {"message":"Account created. Verify your email before signing in.","user":_user_payload(user),"verification_required":True}
@router.get("/me")
...
def api_catalog(q:str="",genre:str="",page:int=1,limit:int=20,db:Session=Depends(get_db)):
    ...
    if q.strip():term=f"%{q.strip()}%";query=query.filter(or_(Track.title.ilike(term),Track.genre.ilike(term),Track.tags.ilike(term)))
...
def api_order(order_id:str,db:Session=Depends(get_db),user:User=Depends(require_user)):
    ...
    return {"id":order.id,"order_number":order.order_number,"status":getattr(order.status,"value",order.status),"amount":float(order.gross_amount),"currency":order.currency,"track":_track_payload(order.track) if order.track else None,"license_id":getattr(lic,"id",None),"download_available":order.status==OrderStatus.COMPLETED and lic is not None}
...
async def api_paystack_initialize(payload:PaymentIn,db:Session=Depends(get_db),user:User=Depends(require_user)):
    ...
    db.add(order);db.flush();request_payload={"email":payload.email or user.email,"amount":int(price*100),"currency":currency,"reference":order.order_number,"callback_url":f"{settings.BASE_URL.rstrip('/')}/paystack/callback","channels":["card","mobile_money"],"metadata":{"beathub_order_id":order.id,"beathub_track_slug":track.slug,"buyer_id":user.id}}
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15.0,connect=5.0)) as client:response=await client.post(f"{settings.PAYSTACK_BASE_URL.rstrip('/')}/transaction/initialize",headers={"Authorization":f"Bearer {settings.PAYSTACK_SECRET_KEY}","Content-Type":"application/json"},json=request_payload);data=response.json()
    except Exception:db.rollback();raise HTTPException(502,"Paystack could not be reached. Please try again.")
