...
router = APIRouter(prefix="/admin/growth-acquisition", tags=["growth-acquisition-v6"])
...
def _queue_snapshot(db: Session, limit: int = 20) -> list[dict]:
    rows = (
        db.query(GrowthProspect)
        .filter(GrowthProspect.status.in_(list(TRACKABLE_STAGES)))
...
async def acquisition_page(request: Request, db: Session = Depends(get_db), admin=Depends(require_admin)):
    return templates.TemplateResponse(request, "growth_acquisition_v6.html", {
        "request": request,
        "current_user": admin,
...
@router.get("/queue")
async def acquisition_queue(db: Session = Depends(get_db), admin=Depends(require_admin)):
    return JSONResponse({"ok": True, "queue": _queue_snapshot(db)})


@router.post("/run")
async def acquisition_run(
    ...
    result["queue_snapshot"] = _queue_snapshot(db)
    return JSONResponse(result)


@router.post("/prospects/{prospect_id}/contacted")
async def mark_contacted(prospect_id: str, request: Request, db: Session = Depends(get_db), admin=Depends(require_admin)):
...
async def mark_stage(prospect_id: str, request: Request, db: Session = Depends(get_db), admin=Depends(require_admin)):
    prospect = db.query(GrowthProspect).filter(GrowthProspect.id == prospect_id).first()
    if not prospect:
        return JSONResponse({"ok": False, "error": "Prospect not found."}, status_code=404)
    body = await request.json()
