...
type Dashboard = {
  ...
  stats: { total_sales:number; gross_revenue:number; platform_commission:number; net_earnings:number; available_balance:number; pending_withdrawal:number };
  tracks: { id:string; title:string; price:number; currency:string; is_sold:boolean; is_published?:boolean }[];
};
...
export default function CreatorDashboard() {
  ...
  const load=useCallback(async()=>{
    setError('');
    try { setData(await api<Dashboard>('/creator/dashboard')); }
    catch(e){ setError(e instanceof Error?e.message:'Unable to load creator dashboard.'); }
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);load();}} />}>
    <Text style={s.eyebrow}>CREATOR STUDIO</Text>
    <Text style={s.title}>{data.profile.stage_name || user?.stage_name || 'Creator'}</Text>
    <Text style={s.muted}>Manage your music, sales and earnings.</Text>

    <View style={s.grid}>
      <View style={s.stat}><Text style={s.label}>Available</Text><Text style={s.value}>{money(data.stats.available_balance,'KES')}</Text></View>
      <View style={s.stat}><Text style={s.label}>Net earnings</Text><Text style={s.value}>{money(data.stats.net_earnings,'KES')}</Text></View>
