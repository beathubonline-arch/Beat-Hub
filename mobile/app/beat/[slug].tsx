  ...
  return (
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.container}>
        ...
        <View style={s.art}><Text style={s.note}>♪</Text></View>
        ...
        <View style={s.meta}>
          ...
          <Text style={s.metaText}>{track.sales_model}</Text>
        </View>
        {!!track.description && <Text style={s.description}>{track.description}</Text>}
        {!!error && <Text style={s.error}>{error}</Text>}
        <Pressable style={s.buy} onPress={buy} disabled={buying}>
          {buying ? <ActivityIndicator /> : <Text style={s.buyText}>Buy · {track.currency} {track.price.toFixed(2)}</Text>}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  art: { height: 300, borderRadius: 22, backgroundColor: '#211c29', alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  ...
  buy: { backgroundColor: '#fff', padding: 17, borderRadius: 13, alignItems: 'center', marginTop: 28 },
  buyText: { fontWeight: '800', color: '#0d0b12' },
  error: { color: '#ff8f8f', padding: 20 },
});
