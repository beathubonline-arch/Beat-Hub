...
def upgrade() -> None:
    ...
    if "payment_transactions" not in tables:
        raise RuntimeError("Schema guard refused to continue: payment_transactions is missing.")

    columns = {c["name"]: c for c in inspector.get_columns("payment_transactions")}
    ...
    if "character varying" not in status_type and "varchar" not in status_type:
        raise RuntimeError("Schema guard refused to continue: payment_transactions.status must be VARCHAR-backed.")
    ...
    callback = {c["name"]: c for c in sa.inspect(bind).get_columns("payment_transactions")}["callback_processed"]
